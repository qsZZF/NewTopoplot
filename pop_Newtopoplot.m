...
if nargin<2
    ...
    if ~isempty(structout.topo_clim)
        ...
        if size(topo_clim(:),1)~=2
           warning('color limit should be a vector of dimension 2, use [0 1] instead'); 
           topo_clim = [0 1];
        end
        topo_clim = [min(topo_clim) max(topo_clim)];
    else
        topo_clim = [0 1];              
    end
    if ~isempty(topoValue)
        ...
        istopo = 1;
%         topoValue = (topoValue-min((topoValue)))./(max((topoValue))-min((topoValue)));
%         topoValue = topoValue*abs(diff(topo_clim))+topo_clim(1);
    end
