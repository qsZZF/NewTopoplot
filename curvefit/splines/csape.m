...
%   Copyright 1987-2018 The MathWorks, Inc.

%     Generate the cubic spline interpolant in ppform.

if nargin > 2
    conds = convertStringsToChars(conds);
end
...
if ischar(conds)
   if startsWith(conds,'c')
       conds = [1 1];
   elseif startsWith(conds,'n')
       pp = csapi(x,y);
       return
   elseif startsWith(conds,'p')
       conds = [0 0];
   elseif startsWith(conds,'s')
       conds = [2 2];
   elseif startsWith(conds,'v')
       conds = [2 2];
       valconds = zeros(yd,2);
   else
       error(message('SPLINES:CSAPE:unknownends', conds))
   end
end
